...
class ADA:
    def camera_and_image(self, user_prompt="What do you see?"):
        """
        Captures an image and sends it with the user's prompt to Gemma3:4b for multimodal response.
        """
    ...
    def simulate_arm_action(self, action, object_name):
        """
        Simulates a robotic arm performing the specified action on the given object.
        """
        if action.lower() == "pick":
            print(f"Simulating: Robotic arm moves to {object_name}, grasps it, and lifts it up.")
            return f"Robotic arm simulated: Picked up the {object_name}."
        elif action.lower() == "place":
            print(f"Simulating: Robotic arm places the {object_name} at the designated location.")
            return f"Robotic arm simulated: Placed the {object_name}."
        else:
            print(f"Simulating: Robotic arm attempts unknown action '{action}' on {object_name}.")
            return f"Robotic arm simulated: {action} {object_name}."

    def process_command(self, command):
        """
        Takes a command (e.g., 'pick the pen'), captures image, sends to LLM, and simulates arm if object is found.
        """
        image_path = camera.capture_image()
        if image_path is None:
            print("Image capture failed.")
        ...
        llm_reply = response['message']['content']
        print("Vision model response:", llm_reply)
        
        # Simple object detection: adjust logic for more complex parsing if needed
        if ("pen" in llm_reply.lower()) and ("see" in llm_reply.lower() or "found" in llm_reply.lower() or "visible" in llm_reply.lower()):
            # Pen is visible, simulate arm action
            result = self.simulate_arm_action("pick", "pen")
        else:
            result = "No pen is there."
            print(result)
        return result
    ...
    def __init__(self):
        ...
        self.instruction_prompt_with_function_calling = '''
            ...
            ```python
            def camera_and_image(user_prompt):
                """Captures an image and sends it with the user's prompt to Gemma3:4b for multimodal response."""
            ...
            def process_command(command):
                """
                Simulates a robotic arm performing an action (e.g., 'pick the pen').
                Captures an image, sends it to the vision model with the command, and simulates arm movement if object is found.
                Only call when the user gives a command to interact with a real object.
                """
            ```
        User: {user_message}
        '''
                    ...
                    for chunk in response:
                        ...
                        if chunk_content == "```":
                            if in_function_call == True:
                                in_function_call = False
                                function_call += "```"
                                tool_output = self.extract_tool_call(function_call, prompt)
    ...
    def extract_tool_call(self, text, user_message=""):
        import io
        ...
        if match:
            ...
            with redirect_stdout(f):
                if code.startswith("camera_and_image(") or code.startswith("self.camera_and_image("):
                    result = eval(f"self.camera_and_image('{user_message}')")
                elif code.startswith("process_command(") or code.startswith("self.process_command("):
                    result = eval(f"self.process_command('{user_message}')")
                else:
                    result = eval(code)
            output = f.getvalue()
